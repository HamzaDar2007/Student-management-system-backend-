...
export class MailService {
    ...
    async sendMail(to: string, subject: string, html: string) {
        try {
            const info = await this.transporter.sendMail({
                from: this.configService.get<string>('MAIL_FROM') || '"Student Management" <noreply@example.com>',
                to,
                subject,
                html,
            });
    ...
    async sendWelcomeEmail(user: any) {
        ...
        `;
        return this.sendMail(user.email, 'Welcome to Student Management System', html);
    }
